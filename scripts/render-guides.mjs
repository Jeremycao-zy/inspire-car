...
// 最终机位：正面、侧前（右）、正侧、正后、侧后（右）
// 模型朝向：+X 为车头，-X 为车尾；az 正值相机在 +Z 侧。
// ⚠️ 用户反馈（2026-09-17）：「右前」机位为 az=-45（车头朝画面左）；
//    az=45 曾被误判为右前，用户实拍对比后判定为左前视角，已弃用。
const shots = [
  [0, 'front.png'],      // 正面：车头正对镜头
  [-45, 'front-right.png'], // 侧前（右）：用户确认机位，车头朝画面左
  [90, 'side.png'],      // 正侧
  [180, 'rear.png'],     // 正后：车尾正对镜头
  [135, 'rear-right.png'], // 侧后（右）
];
